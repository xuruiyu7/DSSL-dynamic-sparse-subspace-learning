...
if __name__ == '__main__':
    ...
    for j in range(num_data):
        ...
        data_noise=data+np.random.normal(size=(128,n_signals))*sigma2
...
#%%    
    ...
    true_location=np.zeros((128))
    true_location[33:65]=32
    true_location[65:128]=64
    ...
    plt.plot(true_location,label='true location',color='purple',ls='-',lw=2)
    plt.axvline(32,color='k',ls=(10,(5,4)),lw=1)
    plt.axvline(64,color='k',ls=(10,(5,4)),lw=1)
    plt.axhline(0,color='k',ls=(10,(5,4)),lw=1)
    plt.axhline(32,color='k',ls=(10,(5,4)),lw=1)
    plt.axhline(64,color='k',ls=(10,(5,4)),lw=1)
...
#%%    
    ...
    for i in range(n_signals):
        ...
        coef_matrix_large[6:12,:]=np.transpose(coef_matrix[0:n_signals,2])
        plt.figure()
        plt.imshow(np.transpose(coef_matrix_large),vmax=1,vmin=-1,cmap='PuOr')
        plt.axhline(19.5,color='k',ls=(10,(5,4)),lw=1)
        #x = range(0,8,2)
        #plt.xticks(x,(0,32,64,96))
        #plt.colorbar()
        plt.xticks([])
        plt.yticks([])
...
#%%    
    ...
    print("Recall: %.2f%%, %.2f%%" %(np.mean(Recall_list)*100,np.std(Recall_list)*100))
...
#%%    
    ...
    for i in range(n_experiments):
        ...
        cp_OP,list_t_OP,list_R_K_OP=PELT_select_y_time(data_noise,lambda1,lambda2,-100,select_y_list,5)
    ...
    for i in range(n_experiments):
        data=simulation(n_length,n_signals)
        data_noise=data+np.random.normal(size=(n_length,n_signals))*.05
        cp_K_2,list_t_K_2,list_R_K_2=PELT_select_y_time(data_noise,lambda1,lambda2,K,select_y_list,5)
    ...
    np.save('list_t_OP_list.npy',np.array(list_t_OP_list))
    #np.save('list_t__list.npy',np.array(list_t_0_list))
    ...
    plt.plot(33+np.arange(0,32,1),np.ones(32)*128-32,color='r',linestyle=(0,(3,2)),lw=1.5)
    plt.plot(65+np.arange(0,64,1),np.ones(64)*128-64,color='r',linestyle=(0,(3,2)),lw=1.5)
